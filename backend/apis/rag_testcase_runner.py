...
def _generate_allure_report(results_dir: Path, report_dir: Path, cwd: Path, env: dict[str, str]) -> None:
    ...
    if result.returncode != 0:
        ...
        raise HTTPException(status_code=500, detail=f"Allure report generation failed: {detail.strip()}")
...
def _flatten_planned_tests(planned: list[dict]) -> list[str]:
    ...
    for item in planned or []:
        if isinstance(item, dict):
            tests.extend([str(name) for name in (item.get("tests") or []) if str(name).strip()])
    return tests


def _evaluate_ac_from_metadata(meta_dir: Path, executed_tests: Optional[list[str]] = None) -> dict:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.goto(site_url)
            result = evaluate_acceptance_criteria(page, acceptance_criteria, executed_tests=executed_tests)
            browser.close()
        return result
...
def run_latest_generated_story_test(
    ...
    try:
        ...
        if ac_result is None and first_meta_dir is not None:
            ...
            ac_result = _evaluate_ac_from_metadata(first_meta_dir, executed_tests=executed_tests)
            if first_storage is not None:
                ac_path = first_meta_dir / "ac_results.json"
                _write_with_storage(ac_path, json.dumps(ac_result, indent=2), first_storage)
